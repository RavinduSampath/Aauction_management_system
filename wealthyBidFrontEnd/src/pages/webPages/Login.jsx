...
const LoginForm = ({ user, setUser }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("https://localhost:7216/api/users/login", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#98FF98] p-4">
      <motion.div
      ...
      >
        <motion.h2
          ...
          whileHover={{ rotate: 2 }}
        >
          <span className="text-yellow-400">Wealthy</span>{" "}
          <span className="text-[#000435]">Bid</span>
        </motion.h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <motion.button
            type="submit"
            className="w-full px-4 py-3 font-bold text-white transition-transform bg-green-900 rounded-lg shadow-lg hover:bg-yellow-400 focus:outline-none focus:ring-4 focus:ring-gray-600"
