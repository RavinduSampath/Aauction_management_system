...
const CreateAuction = () => {
  ...
  const uploadImage = async (image) => {
    ...
    const uploadResponse = await fetch(
      "https://localhost:7216/api/auctions/upload-image",
      {
        method: "POST",
        body: formDataImage,
      }
    ...
    return `https://localhost:7216/uploads/${uploadResult.data.fileName}`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://localhost:7216/api/auctions/create",
        {
          method: "POST",
          headers: {
      ...
      await Promise.all(
        additionalImages.map(async (imageData, index) => {
          if (images[index + 1]) {
            ...
            await fetch("https://localhost:7216/api/auctions/additem", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
